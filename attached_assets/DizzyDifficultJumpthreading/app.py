...
def admin_pre_approve_visitor():
    ...
    visitor = VisitorLog(
        ...
        is_pre_approved_service=True
    )
    db.session.add(visitor)
    db.session.commit()
    
    return jsonify({'success': True, 'message': 'Visitor pre-approved', 'visitor_id': visitor.id})
...
def get_notifications():
    notifications = ActivityLog.query.filter(
        ...
        ((ActivityLog.society_name == current_user.society_name) & (ActivityLog.user_type == 'notification'))
