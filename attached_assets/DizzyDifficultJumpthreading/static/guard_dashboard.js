        ...
        form.addEventListener('submit', (e) => {
            ...
            newItem.innerHTML = `
                ...
                <div class="member-info" style="flex-grow: 2;">
                    ...
                    <div class="member-detail">To: Flat ${flatNo} • Requested: ${entryTime}</div>
                </div>
                <div class="member-actions" style="display:flex; gap:8px;">
                    <button class="btn-approve" style="background:#d4edda; color:#155724; padding:6px 12px; border-radius:4px; cursor:pointer; border:1px solid #c3e6cb;" onclick="approveVisitor(this, '${visitorName}', '${flatNo}')">Allow</button>
                    <button class="btn-deny" style="background:#f8d7da; color:#721c24; padding:6px 12px; border-radius:4px; cursor:pointer; border:1px solid #f5c6cb;" onclick="denyVisitor(this, '${visitorName}', '${flatNo}')">Deny</button>
                </div>
            `;
            ...
            addActivity(`Permission request: ${visitorName} to Flat ${flatNo}`);
            form.reset();
...
function markVisitorExit(visitorId) {
    fetch(`/api/visitor-log/${visitorId}/exit`, { method: 'POST' })
        .then(res => res.json())
        .then(data => {
            if (data.success) {
