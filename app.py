...
class User(UserMixin, db.Model):
    ...
    in_out_status = db.Column(db.String(10), default='out')
...
def get_maintenance_requests():
    ...
    if current_user.user_type == 'resident':
        if current_user.role == 'admin':
            query = query.filter_by(society_name=current_user.society_name, is_public=True)
        else:
            query = query.filter(
                (MaintenanceRequest.created_by_id == current_user.id) |
                ((MaintenanceRequest.is_public == True) & 
...
def delete_maintenance_request(request_id):
    ...
    if current_user.role != 'admin' and maintenance_request.created_by_id != current_user.id:
        return jsonify({'success': False, 'message': 'Unauthorized'}), 403
...
def get_residents_for_guard():
    ...
    for res in residents:
        ...
        result.append({
            ...
            'in_out_status': res.in_out_status or 'out',
            'family_members': [{
                'id': fm.id,
                'name': fm.name,
...
@app.route('/api/guard/resident/<int:resident_id>/status', methods=['POST'])
@login_required
def update_resident_in_out_status(resident_id):
    if current_user.role not in ['admin', 'guard']:
        return jsonify({
            'success': False,
            'message': 'Unauthorized'
    ...
    if status not in ['in', 'out']:
        return jsonify({
            'success': False,
            'message': 'Invalid status. Must be "in" or "out"'
        }), 400
    ...
    resident.in_out_status = status
    db.session.commit()
    
    log_activity(
        f"Resident {status.upper()}",
        f"{resident.full_name} ({resident.flat_number}) marked as {status.upper()}",
        current_user
    )
    
    return jsonify({
        'success': True,
        'message': f'Resident marked as {status.upper()}',
...
def ask_visitor_permission(visitor_id):
    ...
    if resident:
        create_notification(
            user_id=resident.id,
            title='Visitor Permission Request',
            message=f"Visitor {visitor.visitor_name} ({visitor.purpose or 'Guest'}) is at the gate requesting entry to Flat {visitor.flat_number}. Guard: {current_user.full_name}",
            notification_type='visitor_permission',
            society_name=current_user.society_name,
            related_id=visitor.id
        )
...
@app.route('/api/admin/create-bill', methods=['POST'])
@login_required
def admin_create_bill():
    ...
    if not title or not amount or not due_date:
        return jsonify({'success': False, 'message': 'Title, amount, and due date are required'}), 400
    ...
    if not approved_residents:
        return jsonify({'success': False, 'message': 'No approved residents found in this society'}), 400
    ...
    for resident in approved_residents:
        ...
        notification = Notification(
            user_id=resident.id,
            title='New Society Bill',
            message=f'A new bill of ₹{amount} for "{title}" is due on {due_date}',
    ...
    log_activity('Bill Created', f'Admin created bill "{title}" of ₹{amount} for {bills_created} residents', current_user)
    
    return jsonify({
        'success': True,
        'message': f'Bill sent to {bills_created} residents successfully',
...
def guard_mark_visitor_entry():
    ...
    log_activity('Visitor Entry', f"Guard {current_user.full_name} checked in visitor {visitor.visitor_name} for flat {visitor.flat_number}", current_user)
    
    if visitor.resident_id:
        notification_title = 'Visitor Arrived' if visitor.is_pre_approved else 'Visitor Checked In'
        notification_message = f"Your visitor {visitor.visitor_name} has arrived at the gate and has been checked in by {current_user.full_name}."
        ...
        }, room=f"user_{visitor.resident_id}")
...
def get_gates():
    ...
    gates = Gate.query.filter_by(society_name=society_name).all()
    
    if not gates:
        default_gates = [
            Gate(society_name=society_name, gate_name='Main Gate', status='closed'),
            Gate(society_name=society_name, gate_name='Gate B (Parking)', status='closed'),
            Gate(society_name=society_name, gate_name='Emergency Exit', status='closed')
        ]
...
@app.route('/api/gates/<int:gate_id>/status', methods=['POST'])
@login_required
def update_gate_status(gate_id):
    if current_user.role not in ['guard', 'admin']:
        return jsonify({'success': False, 'message': 'Unauthorized'}), 403
    
    gate = Gate.query.get(gate_id)
    if not gate or gate.society_name != current_user.society_name:
        return jsonify({'success': False, 'message': 'Gate not found'}), 404
    ...
    gate.updated_by_id = current_user.id
    gate.updated_by_name = current_user.full_name
    db.session.commit()
    
    log_activity(
        'Gate Status Changed',
        f"{gate.gate_name} {new_status} by {current_user.full_name}",
        current_user
    )
    ...
    for admin in admins:
        notification = Notification(
            ...
            message=f"{gate.gate_name} has been {new_status} by {current_user.full_name}",
            notification_type='gate_status',
            related_id=gate.id,
            society_name=current_user.society_name
        )
        db.session.add(notification)
    ...
    socketio.emit('gate_status_changed', {
        'gate_id': gate.id,
        'gate_name': gate.gate_name,
        'status': new_status,
        'updated_by': current_user.full_name,
        'society_name': current_user.society_name
    }, room=current_user.society_name)
    
    return jsonify({
        'success': True,
        'message': f'{gate.gate_name} is now {new_status}',
        'gate': {
            'id': gate.id,
            'gate_name': gate.gate_name,
            'status': gate.status,
            'updated_at': gate.updated_at.isoformat(),
            'updated_by_name': gate.updated_by_name
        }
    })
...
def get_business_bookings():
    ...
    result = []
    for b in bookings:
        creator = User.query.get(b.created_by_id)
        result.append({
            'id': b.id,
            'title': b.title,
            ...
            'resident_name': creator.full_name if creator else 'Unknown',
            'resident_phone': creator.phone if creator else None,
...
def get_business_stats():
    ...
    earnings = db.session.query(db.func.sum(Earnings.amount)).filter(
        Earnings.provider_id == current_user.id,
        Earnings.status == 'Confirmed',
        Earnings.created_at >= month_start
    ).scalar() or 0
    
    avg_rating_result = db.session.query(db.func.avg(Review.rating)).filter_by(business_id=current_user.id).scalar()
...
def handle_join_user_room(data):
    ...
    if user_id and hasattr(current_user, 'id') and current_user.is_authenticated:
