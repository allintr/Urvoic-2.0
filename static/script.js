...
let selectedBusinessSocieties = [];
let allSocietiesCache = [];
...
async function fetchAllSocieties() {
  try {
    ...
    const data = await response.json();
    if (data.success) {
      allSocietiesCache = data.societies || [];
      return allSocietiesCache;
    }
  } catch (error) {
...
function renderSocietyCheckboxList(societies, searchTerm = '') {
  const listContainer = document.getElementById('societyCheckboxList');
  if (!listContainer) return;

  const filtered = searchTerm
    ? societies.filter(s => s.name.toLowerCase().includes(searchTerm.toLowerCase()))
    : societies;

  if (filtered.length === 0) {
    listContainer.innerHTML = `<div class="society-empty-msg">No societies found. <a onclick="navigateToLogin('register', 'resident')" style="color:var(--brand);cursor:pointer;font-weight:600;">Register your society</a></div>`;
    return;
  }

  listContainer.innerHTML = filtered.map(society => {
    const isChecked = selectedBusinessSocieties.some(s => s.id === society.id);
    return `
      <label class="society-checkbox-item">
        <input type="checkbox" value="${society.id}" data-name="${society.name}" ${isChecked ? 'checked' : ''} onchange="toggleSocietySelection(${society.id}, '${society.name.replace(/'/g, "\\'")}')">
        <div class="society-checkbox-info">
          <div class="society-checkbox-name">${society.name}</div>
          <div class="society-checkbox-address">${society.address || 'Address not available'}</div>
        </div>
      </label>
    `;
  }).join('');
...
function toggleSocietySelection(id, name) {
  const existingIndex = selectedBusinessSocieties.findIndex(s => s.id === id);
  if (existingIndex >= 0) {
    selectedBusinessSocieties.splice(existingIndex, 1);
  } else {
    selectedBusinessSocieties.push({ id, name });
  }
  renderSelectedSocietiesTags();
}

function renderSelectedSocietiesTags() {
  const tagsContainer = document.getElementById('selectedSocietiesTags');
  if (!tagsContainer) return;

  tagsContainer.innerHTML = selectedBusinessSocieties.map(society => `
    <span class="society-tag">
      ${society.name}
      <button type="button" class="society-tag-remove" onclick="removeSocietySelection(${society.id})">&times;</button>
    </span>
  `).join('');
}

function removeSocietySelection(id) {
  selectedBusinessSocieties = selectedBusinessSocieties.filter(s => s.id !== id);
  renderSelectedSocietiesTags();
  renderSocietyCheckboxList(allSocietiesCache, document.getElementById('businessSocietySearchInput')?.value || '');
}

async function initBusinessSocietyMultiSelect() {
  const listContainer = document.getElementById('societyCheckboxList');
  const searchInput = document.getElementById('businessSocietySearchInput');
  
  if (!listContainer || !searchInput) return;

  listContainer.innerHTML = '<div class="society-loading-msg">Loading societies...</div>';
  
  const societies = await fetchAllSocieties();
  renderSocietyCheckboxList(societies);

  searchInput.addEventListener('input', (e) => {
    renderSocietyCheckboxList(allSocietiesCache, e.target.value);
  });
}
...
async function handleBusinessSignup(event) {
  ...
  const inputs = form.querySelectorAll('input:not(#businessSocietySearchInput):not([type="checkbox"])');
  ...
  const businessCategory = inputs[5].value;
  
  const societyNames = selectedBusinessSocieties.map(s => s.name);
  ...
  try {
    const response = await fetch('/api/signup', {
      ...
      body: JSON.stringify({
        full_name: fullName, email, phone, password,
        user_type: 'business', role: 'business', society_name: societyNames.join(', '), flat_number: '',
  ...
  if (currentAuthMode === 'register') {
    ...
    if (currentAuthMode === 'signin') {
      ...
      if (currentUserType === 'resident') {
        ...
        initBusinessSocietyMultiSelect();
      }
    }
  }
