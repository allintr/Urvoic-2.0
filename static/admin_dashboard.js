            ...
            if (data.success && data.activities) {
                ...
                if (list && data.activities.length > 0) {
                    list.innerHTML = data.activities.slice(0, 5).map(act => {
                        ...
                        return `<li><i data-lucide="${icon}" class="activity-icon-sm" style="color: var(--brand);"></i><span class="activity-text">${act.description}</span></li>`;
                    }).join('');
                    lucide.createIcons();
                } else if (list) {
...
function applyMaintenanceFilter() {
    ...
    const activeBtn = filterToggle.querySelector('.filter-btn.active');
...
function preApproveVisitorAdmin() {
    ...
    const submitBtn = document.querySelector('#admin-visitor-form button[type="submit"]');
    if (submitBtn) {
    ...
    .then(data => {
        if (submitBtn) {
            ...
            submitBtn.textContent = 'Pre-Approve Entry';
        }
        if (data.success) {
    ...
    .catch(err => {
        ...
        showToast('Error connecting to server for pre-approval', 'error');
    });
}
...
function displayApprovedResidents(residents) {
    ...
    const filteredResidents = residents.filter(r => r.user_type === 'resident' && r.role !== 'guard');
...
function openCreateBillModal() {
    ...
    if (modal) {
        ...
        const dateInput = document.getElementById('bill-due-date');
        if (dateInput) {
...
function submitCreateBill() {
    ...
    submitBtn.disabled = true;
    submitBtn.innerHTML = '<i data-lucide="loader" style="width: 16px; height: 16px; margin-right: 8px; animation: spin 1s linear infinite;"></i>Sending...';
    lucide.createIcons();
    ...
    if (photoInput && photoInput.files && photoInput.files[0]) {
        ...
        reader.onload = function(e) {
            billData.photo = e.target.result;
            sendBillToServer(billData, submitBtn, originalText);
        };
...
function sendBillToServer(billData, submitBtn, originalText) {
    fetch('/api/admin/create-bill', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
    ...
    .then(data => {
        ...
        if (data.success) {
            showToast(data.message || `Bill sent to ${data.bills_created} residents!`, 'success');
            closeCreateBillModal();
            loadPayments();
        } else {
            showToast(data.message || 'Failed to create bill', 'error');
        }
    })
    .catch(err => {
        submitBtn.disabled = false;
