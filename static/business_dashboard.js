            ...
            if (data.success && data.activities) {
                ...
                if (list && data.activities.length > 0) {
                    list.innerHTML = data.activities.slice(0, 5).map(act => {
                        return `<li><i data-lucide="activity" class="activity-icon-sm" style="color: var(--brand);"></i><span class="activity-text">${act.description}</span></li>`;
                    }).join('');
                    lucide.createIcons();
                } else if (list) {
...
function displayBusinessBookings(bookings) {
    const newRequestsContainer = document.getElementById('booking-new-requests');
    const upcomingContainer = document.getElementById('booking-upcoming-list');
    ...
    const upcoming = bookings.filter(b => b.status === 'in_progress' || b.status === 'approved');
    const history = bookings.filter(b => b.status === 'completed' || b.status === 'denied');
    
    const newTabBtn = document.querySelector('.toggle-btn[data-tab="tab-new"]');
    const upcomingTabBtn = document.querySelector('.toggle-btn[data-tab="tab-upcoming"]');
    const historyTabBtn = document.querySelector('.toggle-btn[data-tab="tab-history"]');
    
    if (newTabBtn) {
        newTabBtn.innerHTML = `<i data-lucide="bell-ring" style="width: 16px;"></i> New Requests (${pending.length})`;
    }
    if (upcomingTabBtn) {
        upcomingTabBtn.innerHTML = `<i data-lucide="calendar" style="width: 16px;"></i> Upcoming (${upcoming.length})`;
    }
    if (historyTabBtn) {
        historyTabBtn.innerHTML = `<i data-lucide="history" style="width: 16px;"></i> History (${history.length})`;
    }
    lucide.createIcons();
    
    if (newRequestsContainer) {
        if (pending.length === 0) {
            newRequestsContainer.innerHTML = '<p style="text-align:center;color:#999;padding:20px;">No pending requests</p>';
        } else {
            newRequestsContainer.innerHTML = pending.map(b => {
                const createdDate = b.created_at ? new Date(b.created_at).toLocaleDateString('en-IN', { day: 'numeric', month: 'short' }) : '';
                return `
                <div class="member-list-item booking-item" data-id="${b.id}">
                    <div class="profile-pic-placeholder-sm" style="background-color: var(--yellow-bg-light);"><i data-lucide="bell-ring"></i></div>
                    <div class="member-info">
                        <div class="member-name">${b.title}</div>
                        <div class="member-detail">Flat ${b.flat_number || 'N/A'} • ${b.society_name || ''}</div>
                        <div class="member-detail" style="font-size:11px;color:#888;">${b.description || ''}</div>
                    </div>
                    <div class="booking-actions-inline">
                        <button class="btn-sm btn-accept" onclick="openScheduleModal(${b.id}, '${b.title.replace(/'/g, "\\'")}')"><i data-lucide="check"></i></button>
                        <button class="btn-sm btn-decline" onclick="declineBooking(${b.id})"><i data-lucide="x"></i></button>
                    </div>
                </div>
            `}).join('');
            lucide.createIcons();
    ...
    if (upcomingContainer) {
        if (upcoming.length === 0) {
            upcomingContainer.innerHTML = '<p style="text-align:center;color:#999;padding:20px;">No upcoming bookings</p>';
        } else {
            upcomingContainer.innerHTML = upcoming.map(b => {
                const scheduleDate = b.scheduled_date || 'TBD';
                const scheduleTime = b.scheduled_time || '';
                return `
                <div class="member-list-item booking-item" data-id="${b.id}">
                    <div class="profile-pic-placeholder-sm" style="background-color: var(--blue-bg);"><i data-lucide="calendar-check"></i></div>
                    <div class="member-info">
                        <div class="member-name">${b.title}</div>
                        <div class="member-detail">Flat ${b.flat_number || 'N/A'} • ${scheduleDate} ${scheduleTime}</div>
                    </div>
                    <div class="booking-actions-inline">
                        <button class="btn-sm btn-complete" onclick="completeBooking(${b.id})"><i data-lucide="check-check"></i> Done</button>
                    </div>
                </div>
            `}).join('');
            lucide.createIcons();
    ...
    if (historyContainer) {
        if (history.length === 0) {
            historyContainer.innerHTML = '<p style="text-align:center;color:#999;padding:20px;">No booking history</p>';
        } else {
            historyContainer.innerHTML = history.map(b => {
                const statusClass = b.status === 'completed' ? 'status-completed' : 'status-denied';
                const statusText = b.status === 'completed' ? 'Completed' : 'Declined';
                const billBtn = b.status === 'completed' && b.created_by_id ? 
                    `<button class="btn-sm btn-bill" onclick="openBillModal(${b.created_by_id}, '${(b.resident_name || 'Resident').replace(/'/g, "\\'")}', '${(b.title || '').replace(/'/g, "\\'")}', '${b.society_name || ''}')"><i data-lucide="receipt"></i> Bill</button>` : '';
                return `
                <div class="member-list-item booking-item" data-id="${b.id}">
                    <div class="profile-pic-placeholder-sm" style="background-color: ${b.status === 'completed' ? 'var(--green-bg)' : 'var(--red-bg)'};"><i data-lucide="${b.status === 'completed' ? 'check-circle' : 'x-circle'}"></i></div>
                    <div class="member-info">
                        <div class="member-name">${b.title}</div>
                        <div class="member-detail">Flat ${b.flat_number || 'N/A'} • ${b.resident_name || 'Resident'}</div>
                    </div>
                    <div class="booking-actions-inline">
                        ${billBtn}
    ...
    if (todayScheduleList) {
        ...
        const todayBookings = upcoming.filter(b => b.scheduled_date === today);
        
        if (todayBookings.length === 0) {
            todayScheduleList.innerHTML = '<li style="color:#999;text-align:center;padding:20px;">No bookings scheduled for today</li>';
        } else {
            todayScheduleList.innerHTML = todayBookings.map(b => `
                <li class="schedule-item">
                    <span class="schedule-time">${b.scheduled_time || 'TBD'}</span>
                    <div class="schedule-info">
                        <span class="schedule-title">${b.title}</span>
                        <span class="schedule-location">Flat ${b.flat_number || 'N/A'}</span>
                    </div>
                </li>
            `).join('');
        }
...
let currentSchedulingBookingId = null;

function openScheduleModal(bookingId, bookingTitle) {
    currentSchedulingBookingId = bookingId;
    ...
    if (modalTitle) {
        modalTitle.textContent = `Schedule: ${bookingTitle}`;
    }
...
function closeScheduleModal() {
    ...
    currentSchedulingBookingId = null;
}

function confirmScheduleBooking() {
    if (!currentSchedulingBookingId) {
    ...
    fetch(`/api/maintenance-requests/${currentSchedulingBookingId}/approve`, {
        method: 'POST',
    ...
    .then(data => {
        if (data.success) {
            showToast('Booking accepted and scheduled!', 'success');
            closeScheduleModal();
            loadBusinessBookings();
            loadBusinessDashboard();
        } else {
...
function initScheduleModal() {
    ...
    if (confirmBtn) {
        confirmBtn.addEventListener('click', confirmScheduleBooking);
    }
...
function acceptBooking(bookingId) {
    openScheduleModal(bookingId, 'Booking');
}
...
let currentBillResidentId = null;
let currentBillSociety = '';

function openBillModal(residentId, residentName, serviceName, societyName) {
    currentBillResidentId = residentId;
    currentBillSociety = societyName;
    
    const modal = document.getElementById('bill-modal');
    const residentLabel = document.getElementById('bill-resident-name');
    const serviceLabel = document.getElementById('bill-service-name');
    const descInput = document.getElementById('bill-description');
    const amountInput = document.getElementById('bill-amount');
    
    if (residentLabel) residentLabel.textContent = residentName;
    if (serviceLabel) serviceLabel.textContent = serviceName;
    if (descInput) descInput.value = serviceName;
...
function sendBill() {
    ...
    const description = descInput?.value || 'Service charge';
    ...
    if (!currentBillResidentId) {
        showToast('No resident selected', 'error');
        return;
    }
    
    fetch('/api/business/send-bill', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
            resident_id: currentBillResidentId,
            society_name: currentBillSociety,
    ...
    .then(data => {
        if (data.success) {
            closeBillModal();
            showToast('Bill sent to resident!', 'success');
        } else {
            showToast(data.message || 'Failed to send bill', 'error');
        }
    })
    .catch(err => showToast('Error sending bill', 'error'));
}
...
function loadBusinessEarnings() {
    fetch('/api/business/earnings')
        .then(res => res.json())
        .then(data => {
            if (data.success) {
                displayBusinessEarnings(data);
            }
        })
        .catch(err => console.log('Error loading earnings'));
}

function displayBusinessEarnings(data) {
    ...
    const pendingListEl = document.getElementById('pending-earnings-list');
    ...
    const confirmedJobs = earnings.filter(e => e.status === 'Confirmed').length;
    if (totalJobsEl) totalJobsEl.textContent = confirmedJobs;
    
    if (goalProgressBar) {
        goalProgressBar.value = totalEarnings;
        goalProgressBar.max = currentGoalMax;
    }
    if (goalProgressText) {
        goalProgressText.innerHTML = `<strong>₹${totalEarnings.toLocaleString('en-IN')}</strong> of ₹${currentGoalMax.toLocaleString('en-IN')} target`;
    }
    
    currentTotalEarnings = totalEarnings;
    currentTotalJobs = confirmedJobs;
    
    const pendingItems = earnings.filter(e => e.status === 'Pending Confirmation');
    const confirmedItems = earnings.filter(e => e.status === 'Confirmed');
    
    if (pendingListEl) {
        if (pendingItems.length === 0) {
            pendingListEl.innerHTML = '<p style="text-align:center;color:#999;padding:20px;">No pending earnings to confirm</p>';
        } else {
            pendingListEl.innerHTML = pendingItems.map(e => {
                const date = e.transaction_date ? new Date(e.transaction_date).toLocaleDateString('en-IN', { day: 'numeric', month: 'short' }) : 'N/A';
                const searchTerm = `${e.request_title || ''} ${e.society_name || ''}`.toLowerCase();
                return `
                    <div class="member-list-item pending-earning" data-earning-id="${e.id}" data-search-term="${searchTerm}">
                        <div class="profile-pic-placeholder-sm" style="background-color: var(--yellow-bg-light);"><i data-lucide="clock"></i></div>
                        <div class="member-info">
                            <div class="member-name">${e.request_title || 'Service Request'}</div>
                            <div class="member-detail">${e.society_name || ''} • ${date}</div>
                        </div>
                        <div class="earning-amount-inline">
                            <span class="amount-value">₹${(e.amount || 0).toLocaleString('en-IN')}</span>
    ...
    if (transactionListEl) {
        if (confirmedItems.length === 0) {
            transactionListEl.innerHTML = '<p style="text-align:center;color:#999;padding:20px;">No confirmed transactions yet</p>';
        } else {
            transactionListEl.innerHTML = confirmedItems.map(e => {
                const date = e.transaction_date ? new Date(e.transaction_date).toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric' }) : 'N/A';
                const searchTerm = `${e.request_title || ''} ${e.society_name || ''}`.toLowerCase();
                return `
                    <div class="member-list-item" data-search-term="${searchTerm}">
                        <div class="profile-pic-placeholder-sm" style="background-color: var(--green-bg);"><i data-lucide="check-circle"></i></div>
                        <div class="member-info">
                            <div class="member-name">${e.request_title || 'Service Request'}</div>
                            <div class="member-detail">${e.society_name || ''} • ${date}</div>
                        </div>
                        <div class="earning-amount-inline">
                            <span class="amount-value" style="color: var(--brand);">+₹${(e.amount || 0).toLocaleString('en-IN')}</span>
                        </div>
                    </div>
                `;
            }).join('');
