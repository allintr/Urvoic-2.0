...
function initRatingStars() {
    ...
    stars.forEach((star, idx) => {
        star.addEventListener('click', () => {
            ...
            ratingMsg.textContent = messages[idx + 1];
...
function openChat(groupName) {
    document.getElementById('residents-chat-view').classList.remove('active-view');
    document.getElementById('individual-chat-view').classList.add('active-view');
...
function sendChatMessage() {
    ...
    newMsg.innerHTML = `
        <div class="chat-bubble-meta sender">You · now</div>
        <div class="chat-bubble sender">${msg}</div>
    `;
...
function displayResidentChatGroups(groups) {
    const container = document.getElementById('resident-chat-groups');
    if (!container) return;
    ...
    container.innerHTML = groups.map(g => `
        <div class="chat-group-item" onclick="openChat('${g.name}', ${g.id})">
    ...
    container.innerHTML = messages.map(m => {
        ...
        return `
            <div class="chat-message ${isOwn ? 'own-message' : 'other-message'}">
                ${!isOwn ? `<span class="sender-name">${m.sender_name}</span>` : ''}
