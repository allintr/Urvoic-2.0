            ...
            if (data.success && data.activities) {
                ...
                if (list && data.activities.length > 0) {
                    list.innerHTML = data.activities.slice(0, 5).map(act => {
                        ...
                        return `<li><span class="activity-item-status ${statusClass}">${act.action}</span> ${act.description}</li>`;
                    }).join('');
                } else if (list) {
    ...
    function setupStarListeners() {
        ...
        stars.forEach((star, idx) => {
            ...
            star.addEventListener('click', () => {
                ...
                if (ratingMsg) ratingMsg.textContent = messages[clickedRating];
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        if (ratingMsg) ratingMsg.textContent = 'Thank you for rating Urvoic!';
                    } else {
...
function loadFamilyMembers() {
    fetch('/api/family-members')
        ...
        .then(data => {
            ...
            if (data.success && data.family_members && list) {
                if (data.family_members.length === 0) {
                    list.innerHTML = '<li style="color: var(--muted); font-size: 13px;">No family members added yet</li>';
                } else {
                    list.innerHTML = data.family_members.map(m => `
                        <li style="margin-bottom: 5px; display: flex; justify-content: space-between; align-items: center;">
                            <span>${m.name} (${m.relationship})${m.age ? ', Age: ' + m.age : ''}</span>
                            <button style="background: none; border: none; color: #e74c3c; cursor: pointer; font-size: 12px;" onclick="deleteFamilyMember(${m.id})">Remove</button>
                        </li>
                    `).join('');
                }
            }
        })
        .catch(err => console.log('Error loading family members'));
}
...
function loadVehicles() {
    fetch('/api/vehicles')
        ...
        .then(data => {
            ...
            if (data.success && data.vehicles && list) {
                if (data.vehicles.length === 0) {
                    list.innerHTML = '<li style="color: var(--muted); font-size: 13px;">No vehicles added yet</li>';
                } else {
                    list.innerHTML = data.vehicles.map(v => `
                        <li style="margin-bottom: 5px; display: flex; justify-content: space-between; align-items: center;">
                            <span>${v.vehicle_type} - ${v.vehicle_number}${v.vehicle_model ? ' (' + v.vehicle_model + ')' : ''}</span>
                            <button style="background: none; border: none; color: #e74c3c; cursor: pointer; font-size: 12px;" onclick="deleteVehicle(${v.id})">Remove</button>
                        </li>
                    `).join('');
                }
            }
...
function showAddFamilyMemberForm() {
    ...
    if (!name) return;
    const relationship = prompt('Relationship (e.g., Wife, Son, Daughter, Mother):');
    ...
    .then(data => {
        if (data.success) {
            ...
            showToast(data.message || 'Failed to add family member', 'error');
        }
    });
}
...
function deleteFamilyMember(memberId) {
    ...
    fetch(`/api/family-members/${memberId}`, { method: 'DELETE' })
        .then(res => res.json())
        .then(data => {
            if (data.success) {
...
function showAddVehicleForm() {
    const type = prompt('Vehicle Type (Car/Bike/Scooter):');
    if (!type) return;
    const number = prompt('Vehicle Number (Plate):');
    if (!number) return;
    ...
    .then(data => {
        if (data.success) {
            ...
            showToast(data.message || 'Failed to add vehicle', 'error');
        }
    });
}
            ...
            <div class="request-card" data-id="${r.id}">
                <div class="request-header">
                    ...
                    <div class="request-actions">
                        ...
                        <button class="btn-delete-request" onclick="deleteMaintenanceRequest(${r.id}, '${r.title.replace(/'/g, "\\'")}')" title="Delete request">
...
function deleteMaintenanceRequest(requestId, requestTitle) {
    if (!confirm(`Are you sure you want to delete "${requestTitle}"?`)) {
        return;
    }
...
function displayServiceProviders(providers) {
    ...
    if (providers.length === 0) {
        container.innerHTML = '<p style="text-align:center;color:#999;padding:20px;">No service providers available in your society</p>';
        return;
    }
    
    const categoryColors = {
        'electrician': 'tag-blue',
        'plumber': 'tag-orange',
        'tutor': 'tag-purple',
        'cleaner': 'tag-green',
        'carpenter': 'tag-brown'
    ...
    container.innerHTML = providers.map(p => {
        const category = (p.business_category || 'other').toLowerCase();
        const tagClass = categoryColors[category] || 'tag-blue';
        return `
            <div class="provider-card category-${category.replace(/\s+/g, '-')}">
                <div class="provider-body">
                    <div class="provider-tag ${tagClass}">${p.business_category || 'Service'}</div>
                    <h3 class="provider-name">${p.business_name}</h3>
                    <p class="provider-owner">Owner: ${p.full_name || 'N/A'}</p>
                    <div class="provider-contact"><i data-lucide="phone"></i> ${p.phone || 'N/A'}</div>
                </div>
                <div class="provider-footer">
                    <div class="provider-rating"><i data-lucide="star" fill="#ffc107" stroke="#ffc107"></i> ${p.avg_rating ? p.avg_rating.toFixed(1) : 'N/A'}</div>
                    <button class="btn-assign" onclick="openAssignmentModal(${p.id}, '${p.business_name.replace(/'/g, "\\'")}', '${(p.business_category || 'Service').replace(/'/g, "\\'")}')">Assign Work</button>
                </div>
            </div>
        `;
    }).join('');
...
function openAssignmentModal(businessId, providerName, type) {
    ...
    list.innerHTML = '<p style="text-align:center;color:#999;">Loading your private requests...</p>';
    ...
    fetch('/api/maintenance-requests?type=private')
        ...
        .then(data => {
            if (data.success) {
                const myReqs = (data.requests || []).filter(r => 
                    r.request_type === 'private' && !r.assigned_business_id
                );
                    ...
                    myReqs.forEach(req => {
                        ...
                        div.innerHTML = `
                            ...
                            <div>
                                ...
                                <div style="font-size:12px; color:#666;">${req.description ? req.description.substring(0, 50) + '...' : ''}</div>
                            </div>
                        `;
        ...
        .catch(err => {
            list.innerHTML = '<p style="color:#999;">Error loading requests</p>';
        });
...
function confirmAssignment() {
    ...
    const requestId = selected.value;
    ...
    .then(data => {
        if (data.success) {
            ...
            showToast(data.message || "Failed to assign work", "error");
        }
    })
    .catch(err => {
        showToast("Error assigning work", "error");
    });
}
...
function handleFormSubmit(event, successMsg) {
    ...
    // -- SPECIAL LOGIC: Visitor Pre-Approval --
    const visitorNameInput = form.querySelector('#visitor-name');
    if (visitorNameInput) {
        const visitorName = visitorNameInput.value;
        const visitorType = form.querySelector('#visitor-type').value;
        const expectedDate = form.querySelector('#visitor-date').value;
        const expectedTime = form.querySelector('#visitor-time').value;
        
        fetch('/api/visitor-log/pre-approve', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
        ...
        .catch(err => {
            btn.classList.remove('loading');
            btn.innerText = originalText;
            showToast('Error pre-approving visitor', 'error');
        });
        return;
    }
    ...
    if (typeof io !== 'undefined') {
        ...
        socket.on('connect', function() {
            ...
            if (window.currentUserId) {
                socket.emit('join_user_room', { user_id: window.currentUserId });
            }
        });
        ...
        socket.on('visitor_arrived', function(data) {
            console.log('Visitor arrived notification:', data);
            showToast(`Your visitor ${data.visitor_name} has arrived!`, 'success');
            addResidentNotification('Visitor Arrived', data.message);
            if (currentView === 'visitor-log-view') {
                loadResidentVisitors();
            }
        });
