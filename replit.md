...
- **Bill Display Fix**:
  - Fixed bill creation to filter residents by `user_type='resident'` instead of `role='resident'`
  - Admins creating bills now correctly send bills to all approved residents (including co-admins)
  - Verified all visitor workflows working correctly:
    1. Admin/Resident pre-approves visitor → stored in database
    2. Guard asks permission → routes to flat owner resident (not admin)
    3. Permission approved → visitor shows in "Currently Inside" section after check-in
    4. Shift report submitted → sends notifications to all admins

- **Dependencies Cleanup**:
  - Removed redundant dependencies from requirements.txt: SQLAlchemy, python-socketio, eventlet
  - Flask-SQLAlchemy and flask-socketio already include these as dependencies
  - Reduced from 16 to 13 core packages

### Previous Updates (December 17, 2025)
- **Visitor Pre-Approval Fix**:
  - Fixed visitor pre-approval form in resident dashboard to actually POST to /api/visitor-log/pre-approve
  - Previously the form only showed a success toast without saving to database
  - Now correctly sends visitor_name, visitor_type, expected_date, expected_time to backend
  - Success/error handling with proper toast notifications and list refresh

- **Resident Maintenance Delete**:
  - Added delete button with trash icon to resident's maintenance request cards
  - Modified /api/maintenance-requests DELETE to allow creators to delete their own requests (not just admins)
  - Includes confirmation dialog before deletion
  - Added proper CSS styling for delete button with hover effects

- **Verified Existing Implementations**:
  - Admin member filtering: Residents/Guards/Services tabs correctly filter by user_type and role
  - Guard-to-Resident visitor routing: Notifications go directly to residents (not admins)
  - Guard resident list: Shows IN/OUT buttons with phone numbers (no emails)
  - Create bill feature: Admin dashboard has working create-bill-modal
